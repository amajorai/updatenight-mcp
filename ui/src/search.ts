...
function renderCard(item: SearchItem): string {
    ...
    <a href="${url}" target="_blank" rel="noopener noreferrer" class="card">
      ...
      <div style="font-weight:600;font-size:14px;color:#fff;margin-bottom:5px;line-height:1.3">${escapeHtml(item.name)}</div>
      <div style="font-size:12px;color:#777;line-height:1.5">${escapeHtml(item.tagline ?? "")}</div>
    </a>
  `;
}
...
function render(data: SearchResponse): void {
	...
	if (!data.items?.length) {
		el.innerHTML = `<div class="empty">No results for <strong>"${escapeHtml(data.query ?? "")}"</strong></div>`;
		return;
	}
    ...
    <div style="color:#555;font-size:12px;margin-bottom:14px">
      ...
      <span style="color:#aaa">"${escapeHtml(data.query)}"</span>
    </div>
